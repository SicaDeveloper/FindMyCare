...
function RegisterNurse() {
    ...
    const handleFileChange = (event) => {
        ...
        if (name === "citizenshipFront") {
            setCitizenshipFront(files[0]);
            if (errors.citizenshipFront) setErrors(prevErrors => ({ ...prevErrors, citizenshipFront: '' }));
        } else if (name === "citizenshipBack") {
            setCitizenshipBack(files[0]);
            if (errors.citizenshipBack) setErrors(prevErrors => ({ ...prevErrors, citizenshipBack: '' }));
        }
    };
    ...
    const handleNurseRegistration = async (event) => {
        ...
        // The schema requires 'experience' and 'reviews'.
        // For now, assuming 'experience' is not collected in this form, send a default.
        // 'reviews' should be initialized by the backend as an empty array.
        submissionData.append('experience', "0"); // Default experience to 0 years. Needs clarification if this should be collected.
        ...
        <Box sx={{
            ...
            <Paper elevation={3}
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2, width: '100%' }}>
                    <Avatar sx={{ width: 80, height: 80, mb: 1 }} /> {/* Larger Avatar */}
                    <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} width={{xs: '100%', md: '80%'}} justifyContent="center">
                        <FormGroup sx={{ flexGrow: 1, minWidth: '200px' }}>
                            <InputLabel htmlFor="fullName">Full Name</InputLabel>
                            <Input id="fullName" name="fullName" value={formData.fullName} onChange={handleChange} error={!!errors.fullName} fullWidth />
                            {errors.fullName && <FormHelperText error>{errors.fullName}</FormHelperText>}
                        </FormGroup>
                        <FormGroup sx={{ flexGrow: 1, minWidth: '200px' }}>
                            <InputLabel htmlFor="panNumber">PAN Number</InputLabel>
                            <Input id="panNumber" name="panNumber" value={formData.panNumber} onChange={handleChange} error={!!errors.panNumber} fullWidth />
                    ...
                    <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} width={{xs: '100%', md: '80%'}} justifyContent="center">
                        ...
                        <FormGroup sx={{ flexGrow: 1, minWidth: '200px' }}>
                            ...
                            <Input id="phone" name="phone" value={formData.phone} onChange={handleChange} error={!!errors.phone} fullWidth />
                ...
                <Box sx={{
                    ...
                    <FormGroup sx={{ width: { xs: '90%', sm: '70%', md: '60%' } }}>
                        ...
                        <Input id="citizenshipFront" type="file" name="citizenshipFront" onChange={handleFileChange} error={!!errors.citizenshipFront} fullWidth />
                    ...
                    <FormGroup sx={{ width: { xs: '90%', sm: '70%', md: '60%' } }}>
                        ...
                        <Input id="citizenshipBack" type="file" name="citizenshipBack" onChange={handleFileChange} error={!!errors.citizenshipBack} fullWidth />
                ...
                <Button type="submit" variant="contained" color="primary" sx={{ mt: 2, py: 1.5, width: { xs: '80%', sm: '60%', md: '50%' }, alignSelf: 'center' }}>
                    Submit Registration
                </Button>
            </Paper>
        </Box>
